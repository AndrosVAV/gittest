...
//Игра змейка часть2(c.252);

var Block = function(col,row){
	this.col = col;//столбец и строка
	this.row = row;
};
//создание обьекта для ячейки
//var sampleBlock = new Block(5,5);
//Метод drawSquare(c.253);
...
//Добавляем метод drawCircle(c.255);
...
Block.prototype.equal = function(otherBlock){
	return this.col === otherBlock && this.row === otherBlock;
					...
					var apple = new Block(3,3);
					var head = new Block(3,3);
					console.log(head.equal(apple));
					console.log(apple.equal(head));
					*/

//Создаем змейку(c.257);
